...
public class AgilePageController {
	...
	public String loadAgileCommunityLibraryDetailPage(@PathVariable("questionId") long questionId,HttpServletRequest request,Model model){
		
		Question question = getQuestionDetail(questionId);
		
		if(StringUtils.isNotBlank(question.getRecommendedSolution())){
			model.addAttribute("showSolutionBox", false);
		}
		else{
			model.addAttribute("showSolutionBox", true);
		}
	...
	public String loadScenariosTab(@RequestParam("tab") String tab,HttpServletRequest request,Model model){
		...
		for(Question question:getQuestionList()){
			...
			if(tab.equals(question.getCategory()) && question.getIsScenario() == true && StringUtils.isNotBlank(question.getRecommendedSolution())){
